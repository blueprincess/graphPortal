...
void Object::sendNormals(GLfloat *normals)
{
    ...
    // Fills the vbo with the normals data, in a GPU memory special for "staticdraw" access
    // (data should be used to draw frequently, but not be changed often)
    glBufferData(GL_ARRAY_BUFFER, this->nbVertices*3*sizeof(GLfloat), normals,
GL_STATIC_DRAW);
    // Specifies from where and how to read the data in the bound buffer
    glVertexAttribPointer(attributeNormal, 3, GL_FLOAT, GL_FALSE, 0, (const
GLvoid *)0);
